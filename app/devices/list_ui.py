        ...
        with table_container:
            for item in page_items:
                with ui.row().classes('w-full items-center no-wrap gap-2 px-2 py-2 border-b border-gray-200 dark:border-gray-700 text-sm hover:bg-gray-50 dark:hover:bg-gray-800'):
                    ui.label(format_value(item.get('nickname'))).classes('w-[18%] truncate text-blue-600 dark:text-blue-300')
                    ui.label(format_value(item.get('userid'))).classes('w-[22%] truncate')
                    ui.label(format_value(item.get('register_time'))).classes('w-[24%] truncate')
                    ui.label(format_value(item.get('next_send_time'))).classes('w-[24%] truncate')
                    ui.label(format_value(item.get('consecutive_fail_days'))).classes('w-[12%]')
                    ui.label(format_value(item.get('remarks'))).classes('w-[20%] truncate')
    ...
    with ui.card().classes('w-full max-w-5xl mx-auto rounded-2xl shadow-lg'):
        ...
        with ui.column().classes('w-full gap-4 px-4 py-4'):
            with ui.row().classes('w-full items-center no-wrap gap-2 px-2 py-2 font-semibold bg-gradient-to-r from-gray-100 to-gray-50 dark:from-gray-800 dark:to-gray-700 rounded-lg text-sm'):
                ui.label('Nickname').classes('w-[18%]')
                ui.label('UserID').classes('w-[22%]')
                ui.label('注册时间').classes('w-[24%]')
                ui.label('下次发送时间').classes('w-[24%]')
                ui.label('连续失败天数').classes('w-[12%]')
                ui.label('备注').classes('w-[20%]')
