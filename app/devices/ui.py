...
def create_device_management_ui():
    ...
    async def clear_form(reset_picker: bool = True):
        if 'doc_id_to_submit' in state:
            state['doc_id_to_submit'].value = ''
    ...
    async def on_device_picked():
        ...
        if not picked_id:
            ui.notify(f'无法找到标签 "{selected_label}" 对应的ID。', color='negative')
            return
        try:
            importlib.reload(config)
    ...
    def generate_fingerprint():
        did = state['did'].value
        if not did:
            ui.notify('请先输入 DeciceID (did)', color='warning')
            return
    ...
    async def save_device():
        if not (state['userid'].value and state['nickname'].value):
            ui.notify('User ID 和 Nickname 不能为空！', color='negative')
            return
