...
from devices.utils import generate_xhs_fingerprint
...
def call_remote_proxy(payload: dict):
...
NETWORK_ERROR_KEYWORDS = ("网络或连接错误", "客户端连接关闭", "获取IP失败", "无法连接到服务器", "连接中断", "网络连接失败", "连接超时", "连接失败", "ConnectionError", "Connection", "ProxyError", "timeout", "超时")
AUTH_ERROR_KEYWORDS = ("无效的 api key", "缺少 api key", "套餐已过期", "次数已用完", "客户已被暂停")
...
def _claim_receiver(user_id_collection):
    ...
    return user_id_collection.find_one_and_update(
        {'$or': [{'claimed': {'$ne': True}}, {'claim_time': {'$lt': stale_before}}]},
...
def client_main_task(sender_device_config, check_user_device_config, db_collections, device_manager_instance):
    userid = sender_device_config['userid']
    user_id_collection = db_collections['user_id']
    receiver_doc = None
    ...
    try:
        for _ in range(config.MAX_RETRY_ATTEMPTS):
            ...
            if receiver_doc:
                break
            logger.warning(f"未能从集合'{config.MONGO_USER_ID_COLLECTION}'获取接收方，等待 {config.USER_ID_POLLING_INTERVAL}秒后重试...")
            time.sleep(config.USER_ID_POLLING_INTERVAL)
        if not receiver_doc:
            raise ValueError(f"在尝试 {config.MAX_RETRY_ATTEMPTS} 次后仍未能获取到接收方。")
        receiver_id = receiver_doc['user_id']
        logger.success(f"[{userid}] 成功获取接收方: {receiver_id}")
        last_exception = None
        for attempt in range(config.MAX_RETRY_ATTEMPTS):
            try:
                message_text = None
                card_template = None
                if config.MESSAGE_SEND_MODE in [config.SEND_MODE_TEXT_ONLY, config.SEND_MODE_CARD_AND_TEXT]:
                    message_text = db_collections['send_text'].aggregate([{'$sample': {'size': 1}}]).next()['text']
                    message_text = message_text + ''.join(random.choice('!@#$%^&*()') for _ in range(random.randint(1, 2)))
                    logger.info(f"[{userid}] 文本消息已加噪，准备发送。")
                if config.MESSAGE_SEND_MODE in [config.SEND_MODE_CARD_ONLY, config.SEND_MODE_CARD_AND_TEXT]:
                    ...
                    if os.path.exists(card_json_path):
                        try:
                            ...
                            logger.error(f"加载卡片模板文件 '{card_json_path}' 失败: {e}，本次私信不发送卡片。")
                    else:
                        logger.warning(f"卡片模板文件 '{card_json_path}' 不存在，本次私信将不发送卡片。")
                if not message_text and not card_template:
                    raise ValueError("消息内容和卡片模板均为空，请检查配置或文件。")
                ...
                if success:
                    ...
                    task_completed = True
                    device_manager_instance.increment_daily_usage(userid)
                    device_manager_instance.update_next_send_time(userid, time.time() + config.SUCCESS_SEND_INTERVAL)
                    device_manager_instance.clear_consecutive_failure(userid)
                    logger.success(f"[{userid}] 本次任务完成，该账号状态已更新。")
                    return
                else:
...
if __name__ == '__main__':
    log_filename_suffix = sys.argv[1] if len(sys.argv) > 1 else "scheduler"
    log_filepath = os.path.join(log_dir, f"client-{log_filename_suffix}.log")
    logger.add(log_filepath, level="INFO", format="{time:YYYY-MM-DD HH:mm:ss}|{level}|{message}", rotation="10 MB", encoding='utf-8')
